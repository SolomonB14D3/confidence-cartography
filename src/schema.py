...
class TokenAnalysis:
    ...
    top1_rank: int                         # Rank of this token in model's distribution (0=top)
...
class ConfidenceRecord:
    ...
    category: str                          # E.g. "simple_fact", "false_statement"
    label: str                             # Short human label, e.g. "capital_france"
    ...
    with open(path) as f:
        for line in f:
            ...
            if not line:
                continue
            d = json.loads(line)
            # Reconstruct nested TokenAnalysis objects
